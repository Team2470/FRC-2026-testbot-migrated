...
public class LinearServo extends Servo{
    ...
    public LinearServo(int channel, int length, int speed) {
        super(channel);
        super.setBoundsMicroseconds(2, 1, 1, 1, 1); 
        m_speed = speed;
    }

    public void extendActuator(){
        extendPos = getPosition() + 1;
        setPosition(extendPos);
    }
    
    public void retractActuator(){
        retractPos = getPosition() - 1;
    ...
    /**
     * Current position of the servo, must be calling {@link #updateCurPos()} periodically 
     * 
    ...
    public double getPosition(){
      ...
      /**
     * Checks if the servo is at its target position, must be calling {@link #updateCurPos()} periodically 
